...
string which_network(string network)
{
	if (network.find("SecureML") != std::string::npos)
		...
		return "VGG16";
	else if (network.find("New") != std::string::npos)
		return "New";
	else if (network.find("Trans") != std::string::npos)
		return "Trans";
	else if (network.find("ResNet") != std::string::npos)
		return "ResNet";
	else 
...
extern void funcReconstructBit(const RSSVectorSmallType &a, vector<smallType> &b, size_t size, string str, bool print);
void print_vector(RSSVectorSmallType &var, string type, string pre_text, int print_nos)
{
	size_t temp_size = var.size();
	vector<smallType> b(temp_size);
	funcReconstructBit(var, b, temp_size, "anything", false);
	cout << pre_text << endl;
	for (int i = 0; i < print_nos; ++i){
		cout << (bool)b[i] << " ";
		// if (i % 10 == 9)
...
{
// #if (USE_CUDA)
// 	matrixMultRSS_Cuda(a,b, temp3, 
// 					 rows,  common_dim,  columns,
// 				 	 transpose_a,  transpose_b);
// #else
#if (!USING_EIGEN)
/********************************* Triple For Loop *********************************/
	const RSSVectorMyType triple_a = a;
	const RSSVectorMyType triple_b = b;
...
void vectorMultRSS(const RSSVectorMyType &a, vector<myType> &temp3, 
	...
	for (int i = 0; i < rows; ++i)
		{
			for (int j = 0; j < columns; ++j)
			{
				...
				{
					temp3[i*columns*columns + j*columns+k] = a[i*columns + k].first * a[i*columns + j].first +
											a[i*columns + k].first * a[i*columns + j].second +
											a[i*columns + k].second * a[i*columns + j].first;
					temp3[i*columns*columns + j+k*columns] = temp3[i*columns*columns + j*columns+k];
				}
			}
		}


	cpu.CPU_Mul(rows*columns*columns*3);
	cpu.CPU_Add(rows*columns*2);
 

}

void computeExp(const vector<myType> &a, vector<longType> &m, vector<longType> &q, size_t size){
	longType m_tmp;
	int q_temp;
	double temp;
	double ln2 = log(2);
	...
	for (int i = 0; i < size; i++){
		temp = (double)a[i]*pow(2,-FLOAT_PRECISION)/ln2;
		q[i]=floor(temp);
		temp = pow(2,temp-q[i]);
		m[i] = frexp(temp, &q_temp)*M_MAX;
    q[i] = q[i]+q_temp;
	}
}
...
void PositionalEncoding(RSSVectorMyType &vec, int d_model, size_t rows, size_t columns){
	// position = torch.arange(max_len).unsqueeze(1)
	// div_term = torch.exp(torch.arange(0, d_model, 2) * (-log(10000.0) / d_model))
	for(int i = 0; i < rows; i ++){
		for (int j = 0 ; j < columns; j++){
			vec[i].first = vec[i].first + (j%2)?cos(j * exp(j*2*(-log(10000.0) / d_model))):
			sin(j * exp(j*2*(-log(10000.0) / d_model)));
		}
	}
...
void zeroPad(const RSSVectorMyType &a, RSSVectorMyType &b, 
			size_t iw, size_t ih, size_t P, size_t Din, size_t B,bool plus1)
{
	size_t size_B,size_Din,size_w;
	if(plus1==0){
		size_B 	= (iw+2*P)*(ih+2*P)*Din;
		size_Din = (iw+2*P)*(ih+2*P);
		size_w 	= (iw+2*P);
	}
	else{
		size_B 	= (iw+2*P+1)*(ih+2*P+1)*Din;
		size_Din = (iw+2*P+1)*(ih+2*P+1);
		size_w 	= (iw+2*P+1);
	}
	...
	for (size_t i = 0; i < B; ++i)
		for (size_t j = 0; j < Din; ++j) 
			for (size_t k = 0; k < ih; ++k)
				...
				{
					// if((i*size_B + j*size_Din + (k+P)*size_w + l + P>=b.size())||(i*Din*iw*ih + j*iw*ih + k*iw + l>=a.size()))
					// 	cout<< "\n i:" << i << "\n j" << j << "\n k" << k << "\n l" << l << "\n b.size()" <<  b.size()
					// 	<< "\n index:"<< i*size_B + j*size_Din + (k+P)*size_w + l + P <<  "\n a.size()" <<  a.size()
					// 	<< "\n index:"<< i*Din*iw*ih + j*iw*ih + k*iw + l << endl;
