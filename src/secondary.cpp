...
void train(NeuralNetwork* net, string network)
{
	log_print("train");
	...
	for (int i = 0; i < NUM_ITERATIONS; ++i)
	{
		...
		net->getAccuracy(maxIndex, counter, network);
...
void test(bool PRELOADING, string network, NeuralNetwork* net)
{
	...
	// print_vector((*(net->layers[NUM_LAYERS-2])->getActivation()), "FLOAT", "Last layer output:", 300);
	// print_vector((*(net->layers[NUM_LAYERS-1])->getActivation()), "FLOAT", "MPC Output over uint32_t:",300);
	...
	if (PRELOADING)
	{
		...
		// cout<<"MINI_BATCH_SIZE * LAST_LAYER_SIZE"<<MINI_BATCH_SIZE <<" "<< LAST_LAYER_SIZE<<endl;
...
void preload_network(bool PRELOADING, string network, NeuralNetwork* net)
{
	...
	if (which_network(network).compare("SecureML") == 0)
	{
		...
		for (int column = 0; column < 128; ++column)
		{
			for (int row = 0; row < 128; ++row)
			{
				...
				(*((FCLayer*)net->layers[1])->getWeights())[128*row + column] = 
						std::make_pair(floatToMyType(temp_next), floatToMyType(temp_prev));
			}
		}
		...
		for (int column = 0; column < 10; ++column)
		{
			for (int row = 0; row < 128; ++row)
			{
				...
				(*((FCLayer*)net->layers[2])->getWeights())[10*row + column] = 
						std::make_pair(floatToMyType(temp_next), floatToMyType(temp_prev));
			}
		}
		...
		for (int i = 0; i < 128; ++i)
		{
			...
			(*((FCLayer*)net->layers[1])->getBias())[i] = std::make_pair(floatToMyType(temp_next), floatToMyType(temp_prev));
		}
	...
	else if (which_network(network).compare("Sarda") == 0)
	{
		...
		for (int column = 0; column < 100; ++column)
		{
			...
			{
				f_weight2_1 >> temp_next; f_weight2_2 >> temp_prev;
				(*((FCLayer*)net->layers[1])->getWeights())[100*row + column] = 
						std::make_pair(floatToMyType(temp_next), floatToMyType(temp_prev));
			}
		}
		f_weight2_1.close(); f_weight2_2.close();
	...
	else if (which_network(network).compare("MiniONN") == 0)
	{
		...
		{
			f_weight2_1 >> temp_next; f_weight2_2 >> temp_prev;
			(*((CNNLayer*)net->layers[1])->getWeights())[row] = 
					std::make_pair(floatToMyType(temp_next), floatToMyType(temp_prev));
		}
		f_weight2_1.close(); f_weight2_2.close();
		if (ZEROS)
		...
		for (int column = 0; column < 10; ++column)
		{
			for (int row = 0; row < 100; ++row)
			{
				...
				(*((FCLayer*)net->layers[3])->getWeights())[10*row + column] = 
						std::make_pair(floatToMyType(temp_next), floatToMyType(temp_prev));
			}
		}
		...
		{
			f_bias2_1 >> temp_next; f_bias2_2 >> temp_prev;
			(*((CNNLayer*)net->layers[1])->getBias())[i] = std::make_pair(floatToMyType(temp_next), floatToMyType(temp_prev));
		}
		f_bias2_1.close(); f_bias2_2.close();
		if (ZEROS)
		{
		...
		for (int i = 0; i < 10; ++i)
		{
			...
			(*((FCLayer*)net->layers[3])->getBias())[i] = std::make_pair(floatToMyType(temp_next), floatToMyType(temp_prev));
		}
	...
	
	{
		...
		for (int column = 0; column < 500; ++column)
		{
			...
			{
				f_weight3_1 >> temp_next; f_weight3_2 >> temp_prev;
				(*((FCLayer*)net->layers[2])->getWeights())[500*row + column] = 
						std::make_pair(floatToMyType(temp_next), floatToMyType(temp_prev));
			}
		}
		f_weight3_1.close(); f_weight3_2.close();
	...
	else if (which_network(network).compare("New") == 0)
	{
		string temp = "New";
		/************************** Input **********************************/
		string path_input_1 = default_path+"input_"+to_string(partyNum);
		...
		if (ZEROS)
		{
			generate_zeros("input_1", 784*MINI_BATCH_SIZE, temp);
			generate_zeros("input_2", 784*MINI_BATCH_SIZE, temp);
		}
	...
	else if (which_network(network).compare("ResNet") == 0)
	{
		// float temp_float[3]={0,0,0};
		// ifstream f_weight;
		// string temp = "ResNet";
		// /************************** Input **********************************/
		// // string path_input_1 = default_path+"input_"+to_string(partyNum);
		...
		// /************************** Weight0 **********************************/
		// {
		// 	f_weight.open(default_path+"weight_0",std::ifstream::in);
		// 	CNNLayer* Layerp = ((CNNLayer*)net->layers[0]);
		// 	CNNConfig conf = Layerp->conf;
		// 	int rowsize = conf.filters * conf.filterSize * conf.filterSize * conf.inputFeatures;
		// 	for (int row = 0; row < rowsize; ++row)
		// 	{
		// 		f_weight >> temp_float[0];
		// 		(*(Layerp)->getWeights())[row] = 
		// 				std::make_pair(floatToMyType(temp_float[partyNum]), floatToMyType(temp_float[nextParty(partyNum)]));
		// 	}
		// 	f_weight.close();
		// }
		// /************************** Weight1 **********************************/
		// {
		// 	f_weight.open(default_path+"weight_1",std::ifstream::in);
			
		// 	BNLayer* Layerp = ((BNLayer*)net->layers[1]);
		// 	BNConfig conf = Layerp->conf;
		// 	int rowsize = conf.numBatches;
		// 	for (int row = 0; row < rowsize; ++row)
		// 	{
		// 		f_weight >> temp_float[0];
		// 		Layerp->gamma[row] = 
		...
		// /************************** bias2 **********************************/
		// {
		// 	f_weight.open(default_path+"weight_2",std::ifstream::in);
		...
		// }
		// int count_w = 2;
		// for(int resn = 3; resn <= 6; resn ++){
		// 	int convN = ((ResLayer*)net->layers[resn])->conf.convNum;
		// 	for (int convn=0;convn<convN;convn++){
		// 				/************************** Weight0 **********************************/
		// 		{
		// 			count_w++;
		// 			f_weight.open(default_path+"weight_"+to_string(count_w),std::ifstream::in);
		// 			CNNLayer* Layerp = ((ResLayer*)net->layers[resn])->CNNlayers[convn];
		// 			CNNConfig conf = Layerp->conf;
		// 			int rowsize = conf.filters * conf.filterSize * conf.filterSize * conf.inputFeatures;
		// 			for (int row = 0; row < rowsize; ++row)
		// 			{
		...
		// 			BNLayer* Layerp = ((ResLayer*)net->layers[resn])->BNlayers[convn];
		// 			BNConfig conf = Layerp->conf;
		// 			int rowsize = conf.numBatches;
		// 			for (int row = 0; row < rowsize; ++row)
		// 			{
...
void loadData(string net, string dataset) // Need debug
{
	...
	if(net.compare("Trans")==0){
		INPUT_SIZE = 28000;
		LAST_LAYER_SIZE = 28000;
		TRAINING_DATA_SIZE = MINI_BATCH_SIZE;
		TEST_DATA_SIZE = MINI_BATCH_SIZE;
		LARGE_NETWORK = true;
	}
	else if(net.compare("Word2Vec")==0){
		INPUT_SIZE = 28000;
		LAST_LAYER_SIZE = 200;
		TRAINING_DATA_SIZE = MINI_BATCH_SIZE;
		TEST_DATA_SIZE = MINI_BATCH_SIZE;
	...
	{
		std::string MNIST_data_path ="files/MNIST";
		INPUT_SIZE = 784;
		LAST_LAYER_SIZE = 10;
		TRAINING_DATA_SIZE = 128*4;
		TEST_DATA_SIZE = 128;
		LARGE_NETWORK = false;
		
		filename_train_data_next = MNIST_data_path+"/train_data_"+to_string(partyNum);
		filename_train_data_prev = MNIST_data_path+"/train_data_"+to_string(nextParty(partyNum));
		filename_test_data_next = MNIST_data_path+"/test_data_"+to_string(partyNum);
		filename_test_data_prev = MNIST_data_path+"/test_data_"+to_string(nextParty(partyNum));
		filename_train_labels_next = MNIST_data_path+"/train_labels_"+to_string(partyNum);
		filename_train_labels_prev = MNIST_data_path+"/train_labels_"+to_string(nextParty(partyNum));
		filename_test_labels_next = MNIST_data_path+"/test_labels_"+to_string(partyNum);
		filename_test_labels_prev = MNIST_data_path+"/test_labels_"+to_string(nextParty(partyNum));
	}
	...
	// }
	if (net.compare("Trans")==0){
		trainData = RSSVectorMyType(28000*MINI_BATCH_SIZE);
		trainLabels = RSSVectorMyType(28000*MINI_BATCH_SIZE);
		testData = RSSVectorMyType(28000*MINI_BATCH_SIZE);
		testLabels = RSSVectorMyType(28000*MINI_BATCH_SIZE);//TODO; Randomize?
	}
	else if (net.compare("ResNet")==0){
		trainData = RSSVectorMyType(224*224*3*MINI_BATCH_SIZE);
		trainLabels = RSSVectorMyType(1000*MINI_BATCH_SIZE);
		testData = RSSVectorMyType(224*224*3*MINI_BATCH_SIZE);
		testLabels = RSSVectorMyType(1000*MINI_BATCH_SIZE);
	}
	else if (net.compare("Word2Vec")==0){
		trainData = RSSVectorMyType(28000*MINI_BATCH_SIZE);
		trainLabels = RSSVectorMyType(28000*MINI_BATCH_SIZE);
		testData = RSSVectorMyType(28000*MINI_BATCH_SIZE);
		testLabels = RSSVectorMyType(28000*MINI_BATCH_SIZE);
	}
	else{
		float temp_next = 0, temp_prev = 0;
		ifstream f_next(filename_train_data_next.c_str());

		ifstream f_prev(filename_train_data_prev.c_str());
...
void selectNetwork(string network, string dataset, string security, NeuralNetConfig* config)
{
	...
	if (network.compare("SecureML") == 0)
	{
		...
		FCConfig* l0 = new FCConfig(784, MINI_BATCH_SIZE, 128, 1); 
		// ReLUConfig* l1 = new ReLUConfig(128, MINI_BATCH_SIZE);
		FCConfig* l2 = new FCConfig(128, MINI_BATCH_SIZE, 128, 1); 
		// ReLUConfig* l3 = new ReLUConfig(128, MINI_BATCH_SIZE);
		FCConfig* l4 = new FCConfig(128, MINI_BATCH_SIZE, 10, 1); 
	...
	else if (network.compare("Sarda") == 0)
	{
		...
		NUM_LAYERS = 3;
		WITH_NORMALIZATION = true;
		CNNConfig* l0 = new CNNConfig(28,28,1,5,2,2,0,MINI_BATCH_SIZE,false,true);
		// ReLUConfig* l1 = new ReLUConfig(980, MINI_BATCH_SIZE);
		FCConfig* l2 = new FCConfig(980, MINI_BATCH_SIZE, 100, true);
	...
	else if (network.compare("AlexNet") == 0)
	{
		...
		else if (dataset.compare("CIFAR10") == 0)
		{
			...
			CNNConfig* l0 = new CNNConfig(33,33,3,96,11,4,9,MINI_BATCH_SIZE,true,true,false,3,2);
			...
			CNNConfig* l4 = new CNNConfig(5,5,96,256,5,1,1,MINI_BATCH_SIZE,true,true,false,3,2);
			...
			CNNConfig* l8 = new CNNConfig(1,1,256,384,3,1,1,MINI_BATCH_SIZE,false,true);
			// ReLUConfig* l9 = new ReLUConfig(1*1*384,MINI_BATCH_SIZE);
			CNNConfig* l10 = new CNNConfig(1,1,384,384,3,1,1,MINI_BATCH_SIZE,false,true);
			// ReLUConfig* l11 = new ReLUConfig(1*1*384,MINI_BATCH_SIZE);
			CNNConfig* l12 = new CNNConfig(1,1,384,256,3,1,1,MINI_BATCH_SIZE,false,true);
			// ReLUConfig* l13 = new ReLUConfig(1*1*256,MINI_BATCH_SIZE);

			FCConfig* l14 = new FCConfig(1*1*256,MINI_BATCH_SIZE,256,true);
			// ReLUConfig* l15 = new ReLUConfig(256,MINI_BATCH_SIZE);
			FCConfig* l16 = new FCConfig(256,MINI_BATCH_SIZE,256,true);
			// ReLUConfig* l17 = new ReLUConfig(256,MINI_BATCH_SIZE);
			FCConfig* l18 = new FCConfig(256,MINI_BATCH_SIZE,10,true);
		...
		else if (dataset.compare("ImageNet") == 0)
		{
			...
			CNNConfig* l1 = new CNNConfig(56,56,64,64,5,1,2,MINI_BATCH_SIZE,true,true,false,2,2);
			...
			CNNConfig* l5 = new CNNConfig(28,28,64,128,5,1,2,MINI_BATCH_SIZE,true,true,false,2,2);
			...
			CNNConfig* l10 = new CNNConfig(14,14,256,256,3,1,1,MINI_BATCH_SIZE,true,true,false,2,2);
			// MaxpoolConfig* l11 = new MaxpoolConfig(14,14,256,2,2,MINI_BATCH_SIZE);
			// ReLUConfig* l12 = new ReLUConfig(7*7*256,MINI_BATCH_SIZE);

			FCConfig* l13 = new FCConfig(7*7*256,MINI_BATCH_SIZE,1024,true);
			// ReLUConfig* l14 = new ReLUConfig(1024,MINI_BATCH_SIZE);
			FCConfig* l15 = new FCConfig(1024,MINI_BATCH_SIZE,1024,true);
			// ReLUConfig* l16 = new ReLUConfig(1024,MINI_BATCH_SIZE);
			FCConfig* l17 = new FCConfig(1024,MINI_BATCH_SIZE,200,true);
	...
	else if (network.compare("VGG16") == 0)
	{
		...
		else if (dataset.compare("CIFAR10") == 0)
		{
			NUM_LAYERS = 16;
			WITH_NORMALIZATION = false;
			CNNConfig* l0 = new CNNConfig(32,32,3,64,3,1,1,MINI_BATCH_SIZE,false,true);
			// ReLUConfig* l1 = new ReLUConfig(32*32*64,MINI_BATCH_SIZE);		
			CNNConfig* l2 = new CNNConfig(32,32,64,64,3,1,1,MINI_BATCH_SIZE,true,true,false,2,2);
			// MaxpoolConfig* l3 = new MaxpoolConfig(32,32,64,2,2,MINI_BATCH_SIZE);
			// ReLUConfig* l4 = new ReLUConfig(16*16*64,MINI_BATCH_SIZE);

			CNNConfig* l5 = new CNNConfig(16,16,64,128,3,1,1,MINI_BATCH_SIZE,false,true);
			// ReLUConfig* l6 = new ReLUConfig(16*16*128,MINI_BATCH_SIZE);
			CNNConfig* l7 = new CNNConfig(16,16,128,128,3,1,1,MINI_BATCH_SIZE,true,true,false,2,2);
			// MaxpoolConfig* l8 = new MaxpoolConfig(16,16,128,2,2,MINI_BATCH_SIZE);
			// ReLUConfig* l9 = new ReLUConfig(8*8*128,MINI_BATCH_SIZE);

			CNNConfig* l10 = new CNNConfig(8,8,128,256,3,1,1,MINI_BATCH_SIZE,false,true);
			// ReLUConfig* l11 = new ReLUConfig(8*8*256,MINI_BATCH_SIZE);
			CNNConfig* l12 = new CNNConfig(8,8,256,256,3,1,1,MINI_BATCH_SIZE,false,true);
			// ReLUConfig* l13 = new ReLUConfig(8*8*256,MINI_BATCH_SIZE);
			CNNConfig* l14 = new CNNConfig(8,8,256,256,3,1,1,MINI_BATCH_SIZE,true,true,false,2,2);
			// MaxpoolConfig* l15 = new MaxpoolConfig(8,8,256,2,2,MINI_BATCH_SIZE);
			// ReLUConfig* l16 = new ReLUConfig(4*4*256,MINI_BATCH_SIZE);

			CNNConfig* l17 = new CNNConfig(4,4,256,512,3,1,1,MINI_BATCH_SIZE,false,true);
			// ReLUConfig* l18 = new ReLUConfig(4*4*512,MINI_BATCH_SIZE);
			CNNConfig* l19 = new CNNConfig(4,4,512,512,3,1,1,MINI_BATCH_SIZE,false,true);
			// ReLUConfig* l20 = new ReLUConfig(4*4*512,MINI_BATCH_SIZE);
			CNNConfig* l21 = new CNNConfig(4,4,512,512,3,1,1,MINI_BATCH_SIZE,true,true,false,2,2);
			// MaxpoolConfig* l22 = new MaxpoolConfig(4,4,512,2,2,MINI_BATCH_SIZE);
			// ReLUConfig* l23 = new ReLUConfig(2*2*512,MINI_BATCH_SIZE);

			CNNConfig* l24 = new CNNConfig(2,2,512,512,3,1,1,MINI_BATCH_SIZE,false,true);
			// ReLUConfig* l25 = new ReLUConfig(2*2*512,MINI_BATCH_SIZE);
			CNNConfig* l26 = new CNNConfig(2,2,512,512,3,1,1,MINI_BATCH_SIZE,false,true);
			// ReLUConfig* l27 = new ReLUConfig(2*2*512,MINI_BATCH_SIZE);
			CNNConfig* l28 = new CNNConfig(2,2,512,512,3,1,1,MINI_BATCH_SIZE,true,true,false,2,2);
			// MaxpoolConfig* l29 = new MaxpoolConfig(2,2,512,2,2,MINI_BATCH_SIZE);
			// ReLUConfig* l30 = new ReLUConfig(1*1*512,MINI_BATCH_SIZE);

			FCConfig* l31 = new FCConfig(1*1*512,MINI_BATCH_SIZE,4096,true);
			// ReLUConfig* l32 = new ReLUConfig(4096,MINI_BATCH_SIZE);
			FCConfig* l33 = new FCConfig(4096, MINI_BATCH_SIZE, 4096,true);
			// ReLUConfig* l34 = new ReLUConfig(4096, MINI_BATCH_SIZE);
			FCConfig* l35 = new FCConfig(4096, MINI_BATCH_SIZE, 1000,true);
	...
	else if (network.compare("New") == 0)
	{
		assert((dataset.compare("MNIST") == 0) && "New only over MNIST");
		NUM_LAYERS = 6;
		WITH_NORMALIZATION = true;
		...
		ReLUConfig* l3 = new ReLUConfig(100, MINI_BATCH_SIZE);
		FCConfig* l4 = new FCConfig(100, MINI_BATCH_SIZE, 10);
		SoftmaxConfig* l5 = new SoftmaxConfig(10, MINI_BATCH_SIZE);
		config->addLayer(l0);
		config->addLayer(l1);
		config->addLayer(l2);
		config->addLayer(l3);
	...
	else if (network.compare("ResNet") == 0)
	{
		// assert((dataset.compare("ImageNet") == 0) && "New only over MNIST");
		NUM_LAYERS = 7;
		WITH_NORMALIZATION = false;
		CNNConfig* l0 = new CNNConfig(224,224,3,64,7,2,3,MINI_BATCH_SIZE,1,0,1,3,2);
		// BNConfig * l1 = new BNConfig(112*112*64,MINI_BATCH_SIZE);
		// MaxpoolConfig * l2 = new MaxpoolConfig(112,112,64,3,2,MINI_BATCH_SIZE);
		ResConfig * l3 = new ResConfig(56,MINI_BATCH_SIZE,56,4,2,64,64,3,false);
		ResConfig * l4 = new ResConfig(56,MINI_BATCH_SIZE,28,4,2,128,64,3,true);
		ResConfig * l5 = new ResConfig(28,MINI_BATCH_SIZE,14,4,2,256,128,3,true);
		ResConfig * l6 = new ResConfig(14,MINI_BATCH_SIZE,7,4,2,512,256,3,true);
		AvgConfig * l7 = new AvgConfig(7*7,512*MINI_BATCH_SIZE);
		FCConfig * l8 = new FCConfig(512,MINI_BATCH_SIZE,1000);
		// SoftmaxConfig * l9 = new SoftmaxConfig(1000,MINI_BATCH_SIZE,false);
	...
	else if (network.compare("Trans") == 0)
	{
		// assert((dataset.compare("MNIST") == 0) && "New only over MNIST");
		cout << "start";
		...
		NUM_LAYERS = 8;
		WITH_NORMALIZATION = false;
		EmbConfig* l0 = new EmbConfig(28000,MINI_BATCH_SIZE,200);//TODO: size
		MHAConfig* l1 = new MHAConfig(200, MINI_BATCH_SIZE,200, 2, false);
		FFConfig* l2 = new FFConfig(200, MINI_BATCH_SIZE);
		EmbConfig* l3 = new EmbConfig(28000,MINI_BATCH_SIZE,200);//TODO: size
		MHAConfig* l4 = new MHAConfig(200, MINI_BATCH_SIZE,200, 2, true);
		MHAConfig* l5 = new MHAConfig(200, MINI_BATCH_SIZE,200, 2, false);
		FFConfig* l6 = new FFConfig(200, MINI_BATCH_SIZE);
		FCConfig* l7 = new FCConfig(200, MINI_BATCH_SIZE, 28000);//TODO: size
		// SoftmaxConfig* l8 = new SoftmaxConfig(28000, MINI_BATCH_SIZE, false);
		config->addLayer(l0);
		config->addLayer(l1);
		config->addLayer(l2);
		config->addLayer(l3);
	...
	else if (network.compare("Word2Vec") == 0)
	{
		// assert((dataset.compare("ImageNet") == 0) && "New only over MNIST");
		NUM_LAYERS = 3;
		WITH_NORMALIZATION = false;
		EmbConfig* l0 = new EmbConfig(28000,MINI_BATCH_SIZE,300);//TODO: size
		FCConfig* l1 = new FCConfig(300, MINI_BATCH_SIZE, 200);
		SoftmaxConfig* l2 = new SoftmaxConfig(200, MINI_BATCH_SIZE, false);
		...
		assert(false && "Only SecureML, Sarda, Gazelle, LeNet, AlexNet, VGG16, ResNet, Trans, Word2Vec Networks supported");
