...
:Layer(_layerNum),
 conf(conf->imageHeight, conf->imageWidth, conf->inputFeatures, 
	  ...
	  conf->maxpool, conf->relu,conf->BN,conf->poolSize, conf-> stride_M),
 weights(conf->filterSize * conf->filterSize * conf->inputFeatures * conf->filters),
 biases(conf->filters),
 gamma(conf->batchSize),
 beta(conf->batchSize)
...
// 	    (((conf->imageHeight - conf->filterSize + 2*conf->padding)/conf->stride) + 1)),
{
	...
	if(conf->maxpool){
		...
		activations.resize(conf->batchSize * conf->filters * 
						(neww/conf->stride_M+1) * (newh/conf->stride_M+1) );
		prime.resize(conf->batchSize * conf->filters * 
		    (neww/conf->stride_M+1) * (newh/conf->stride_M+1) *  conf->poolSize*conf->poolSize,make_pair(0,0));
	}
	else{
		activations.resize(conf->batchSize * conf->filters * 
					(((conf->imageWidth - conf->filterSize + 2*conf->padding)/conf->stride) + 1) * 
		...
		prime.resize(conf->batchSize * conf->filters * 
					(((conf->imageWidth - conf->filterSize + 2*conf->padding)/conf->stride) + 1) * 
					(((conf->imageHeight - conf->filterSize + 2*conf->padding)/conf->stride) + 1),make_pair(0,0));
	}
...
void CNNLayer::initialize()
{
	...
	if(partyNum==PARTY_A){
		for (size_t i = 0; i < size; ++i){
			temp[i].first = floatToMyType((float)(rand() % (higher - lower) + lower)/decimation);
			temp[i].second = 0;
		}
	...
	if(partyNum==PARTY_C){
		for (size_t i = 0; i < size; ++i){
			temp[i].second = floatToMyType((float)(rand() % (higher - lower) + lower)/decimation);
			temp[i].first = 0;
		}
		for (size_t i = 0; i < biases.size(); ++i){
...
void CNNLayer::forward(const RSSVectorMyType& inputActivation)
{
	...
	RSSVectorMyType *temp0;
	if((iw - f + 2*P)%S==0)
		temp0 = new RSSVectorMyType ((iw+2*P)*(ih+2*P)*Din*B, make_pair(0,0));
	else
		temp0 = new RSSVectorMyType ((iw+2*P+1)*(ih+2*P+1)*Din*B, make_pair(0,0));

	// if (FUNCTION_TIME):
	// 	cout << "ZP: \t" << funcTime(zeroPad, inputActivation, temp1, iw, ih, P, Din, B) << endl;
	// else
	RSSVectorMyType temp1= *temp0;

		zeroPad(inputActivation, temp1, iw, ih, P, Din, B,(iw - f + 2*P)%S);
	...
	if (conf.maxpool == 0 && conf.relu == 0 &&conf.BN==0){
		...
		// RSSVectorMyType temp3(activations.size());
		funcMatMul_mixed(weights, temp2, activations, prime, Dout, (f*f*Din), (ow*oh*B), 0, 0, FLOAT_PRECISION,
						B, Dout, ow*oh, 1, conf.maxpool, conf.relu, conf.BN, biases, conf.poolSize, conf.stride_M,
						gamma, beta);
